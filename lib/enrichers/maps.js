...
async function getMap(lat, lng, apiKey) {
    const mapW = 900;
    const mapH = 300;
    ...
    const url = `https://maps.googleapis.com/maps/api/staticmap?${params}&key=${apiKey}`;
...
function enricher(dive, options) {
    ...
    return getMap(lat, lng, options.maps.key).then((image) => {
        if (!image) {
            return dive;
        }
        return {
