...
function draw(doc, dive, options) {
    ...
    */
    ...
    const pressureStart =
        typeof gas.pressureStart !== 'undefined'
            ? gas.pressureStart === ''
                ? null
                : Math.floor(gas.pressureStart)
            : '-';
    const pressureEnd =
        typeof gas.pressureEnd !== 'undefined'
            ? gas.pressureEnd === ''
                ? null
                : Math.floor(gas.pressureEnd)
            : '-';
    ...
    const sostaProf = isEmpty ? null : dive.max_depth > 18 ? 2.5 : 0;
    const risalita = Math.ceil(dive.max_depth / 9);
    const durata = isEmpty ? null : dive.bottom_time + sostaProf + risalita + 5;
